...
#
# PA1     110817    EWENHUG         Check the service status of the backup services
#--------------------------------------------------------------------------
#
#   Name       : tc32_syb_check_backupservice.sh
#
#   Description: Generic check file/directory access permission testscript
#
#   Arguments  : Check for following arguments:
#					-d = debug
#					-f = expected service status for SYB_BACKUP
#					-s = expected service status for masterdataservice_BACKUP
#					-n = testcase name
...
do
    case $option in
        d) DEBUG=1;;
        f) SYB_BACKUP_Status=$OPTARG;;
		s) MASTERDATA_Status=$OPTARG;;
        n) TEST_CASE=$OPTARG;;
   esac
done
...
echo "[OUTPUT:TESTCASEDESCRIPTION:Check the service status of the backup services.]"
...
if [ -f /tmp/temp_sybbackupservice.txt ]; then
	rm /tmp/temp_sybbackupservice.txt
fi

$ISQL -Usa -P${SA_PASS} -o/tmp/temp_sybbackupservice.txt<< EOISQL
sp_autoformat sysservers,'srvid,srvstatus,srvname,srvnetname,srvclass,srvsecmech,srvcost,srvstatus2','where srvname like "%BACKUP"'
go
EOISQL


SYB_BACKUP_STATUS=`$CAT /tmp/temp_sybbackupservice.txt | $GREP -i "SYB_BACKUP" | $AWK '{print $2}'`
MASTERDATA_STATUS=`$CAT /tmp/temp_sybbackupservice.txt | $GREP -i "masterdataservice_BACKUP" |$GREP -v "SYB_BACKUP" | $AWK '{print $2}'`


if [ $SYB_BACKUP_STATUS -ne $SYB_BACKUP_Status ]; then
	RETURN_STATUS_REASON="$RETURN_STATUS_REASON Current status of SYB_BACKUP is $SYB_BACKUP_STATUS, expected $SYB_BACKUP_Status"
	RETURN_STATUS=1
fi

if [ $MASTERDATA_STATUS -ne $MASTERDATA_Status ]; then
	RETURN_STATUS_REASON="$RETURN_STATUS_REASON Current status of masterdataservice_BACKUP is $MASTERDATA_STATUS, expected $MASTERDATA_Status"
	RETURN_STATUS=1
fi
