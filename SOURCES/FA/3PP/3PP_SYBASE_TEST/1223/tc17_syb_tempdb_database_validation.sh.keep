...
#
# PA1		090608		EJOSODO			First Version	
# PA2           110601          EWENHUG			Change the return Reason when TSS is not available
#--------------------------------------------------------------------------
#
#   Name       : tc17_syb_tempdb_database_validation.sh 
#
#   Description: Checks that the tempdb database has the correct size, status, data size and log size. 
#
...
echo "[OUTPUT:TESTCASEDESCRIPTION:Checks that the tempdb database has the correct size, status, data size and log size.]"
...
if [ ! "${SA_PASS}" ]; then
	RETURN_STATUS_REASON="${RETURN_STATUS_REASON}; TSS is Unavailable & the Temporary Password file does not exist"
...
if [ -f /tmp/tempdb_overall_size.txt ]; then
rm /tmp/tempdb_overall_size.txt
fi

if [ -f /tmp/tempdb_status.txt ]; then
rm /tmp/tempdb_status.txt
fi

if [ -f /tmp/tempdb_data_size.txt ]; then
rm /tmp/tempdb_data_size.txt
fi

if [ -f /tmp/tempdb_log_size.txt ]; then
rm /tmp/tempdb_log_size.txt
fi

$ISQL -Usa -P${SA_PASS} -w5000 -b -o/tmp/tempdb_overall_size.txt<< EOISQL
set nocount on
go
select sum(size)/512 from sysusages where dbid=2
go
EOISQL

$ISQL -Usa -P${SA_PASS} -w5000 -b -o/tmp/tempdb_status.txt<< EOISQL
set nocount on
go
select status from sysdatabases where name like "tempdb"
go
EOISQL

$ISQL -Usa -P${SA_PASS} -w5000 -b -o/tmp/tempdb_data_size.txt<< EOISQL
set nocount on
go
select sum (size/512) from sysusages where vdevno !=2 and dbid=2 and segmap = 3 and size !=2048
go
EOISQL

$ISQL -Usa -P${SA_PASS} -w5000 -b -o/tmp/tempdb_log_size.txt<< EOISQL
set nocount on
go
select sum (size/512) from sysusages where vdevno !=2 and dbid=2 and segmap = 4 and size !=2048
go
EOISQL

OVERALL_SIZE=`cat /tmp/tempdb_overall_size.txt`
STATUS=`cat /tmp/tempdb_status.txt`
DATA_SIZE=`cat /tmp/tempdb_data_size.txt`
LOG_SIZE=`cat /tmp/tempdb_log_size.txt`

$ECHO "Checking overall size of tempdb database..."
if [ "${OVERALL_SIZE}" -ne $EXPECTED_OVERALL_SIZE ]; then
RETURN_STATUS=1
RETURN_STATUS_REASON="${RETURN_STATUS_REASON}Overall size of tempdb database: $OVERALL_SIZE, does not match with expected overall size: $EXPECTED_OVERALL_SIZE " 
fi

$ECHO "Checking status of tempdb database..."
if [ "${STATUS}" -ne $EXPECTED_STATUS ]; then
RETURN_STATUS=1 
RETURN_STATUS_REASON="${RETURN_STATUS_REASON}Status of tempdb database: $STATUS, does not match with expected status: $EXPECTED_STATUS " 
fi

$ECHO "Checking data size of tempdb database..."
if [ "${DATA_SIZE}" -ne $EXPECTED_DATA_SIZE ]; then
RETURN_STATUS=1 
RETURN_STATUS_REASON="${RETURN_STATUS_REASON}Data size of tempdb database: $DATA_SIZE, does not match with expected data size: $EXPECTED_DATA_SIZE " 
fi

$ECHO "Checking log size of tempdb database..."
if [ "${LOG_SIZE}" -ne $EXPECTED_LOG_SIZE ]; then
RETURN_STATUS=1 
RETURN_STATUS_REASON="${RETURN_STATUS_REASON}Log size of tempdb database: $LOG_SIZE, does not match with expected log size: $EXPECTED_LOG_SIZE " 
fi
