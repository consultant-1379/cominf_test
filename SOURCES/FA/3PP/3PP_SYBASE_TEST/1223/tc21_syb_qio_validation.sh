...
#
# PA1		090615		EJOSODO			First Version	
#--------------------------------------------------------------------------
#
#   Name       : tc21_syb_qio_validation.sh 
#
#   Description: Tests of existence of device and associated link. 
#
...
echo "[OUTPUT:TESTCASEDESCRIPTION:Tests for the existence of device and associated link.]"
...
if [ ! -f /ericsson/syb/conf/list_syb_devices.ksh ]; then
	RETURN_STATUS_REASON="${RETURN_STATUS_REASON}File /ericsson/syb/conf/list_syb_devices.ksh does not exist!"
	echo [OUTPUT:REASON:$RETURN_STATUS_REASON]
	exit 1
else
	./ericsson/syb/conf/list_syb_devices.ksh
fi

NUMBER_OF_LINES=`wc -l $DEVICE_LIST | awk '{print $1}'`
LINE_NUMBER=1

$ECHO "Checking the Sybase Device List..."

while [ $LINE_NUMBER -le $NUMBER_OF_LINES ]
do
	LINE=`awk NR==$LINE_NUMBER $DEVICE_LIST`
	LINK_NAME=`ls -ln $LINE | awk '{print $9" "$10" "$11}'`
	LINK_VALUE=`ls -ln $LINE | awk '{print $3}'`
	if [ $LINK_VALUE -eq 0 ]; then
		RETURN_STATUS=1
		RETURN_STATUS_REASON="${RETURN_STATUS_REASON}QIO link for device missing: $LINK_NAME"
	fi
	LINE_NUMBER=$LINE_NUMBER+1
done

$ECHO "Finished checking the Sybase Device List!"
