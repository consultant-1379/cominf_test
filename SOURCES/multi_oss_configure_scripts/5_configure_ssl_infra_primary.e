#!/usr/bin/expect
#######################################
#SSL configuration on om_infra_primary#
#######################################
...
send "bash /root/multi_oss_configure_scripts/check_ssl_and_root_certificates_infra_prim.sh $om_infra_primary;echo $?\r"
expect {
	-re "0" {
	...
	-re "1" {
		puts "rootca.cer and ssl.crt have not been copied to om_infra_primary"
		send "exit\r"
		expect eof
	}
}
