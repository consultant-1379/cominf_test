...
function createTCSelection(&$db, $available_testcases, $selected_testcases, $errortext='') {
	...
	$html .= "<td align=center>\n";
        $html .= "<table>\n";
        $html .= "<tr>\n";
        $html .= "<td><input type=\"button\" name=\"move_up\" value=\"Up\" onclick=\"moveUp(document.testsuiteform.elements['selected_tcs[]'])\"></td>\n";
        $html .= "</tr>\n";
        $html .= "<tr>\n";
        $html .= "<td><input type=\"button\" name=\"move_dn\" value=\"Dn\" onclick=\"moveDown(document.testsuiteform.elements['selected_tcs[]'])\"></td>\n";
        $html .= "</tr>\n";
        $html .= "</table>\n";
        $html .= "</td>\n";

	$html .= "</tr>\n";
	$html .= "<tr><td colspan=2></td><td colspan=2 style=\"text-align: center;\"><input type=\"button\" name=\"genspec\" value=\"Generate Spec\" onclick=\"OnGenTestSpecButton(document.testsuiteform.elements['selected_tcs[]'])\"></td></tr>\n";
	$html .= "</table>\n";
	return $html;
}
...
if ( $mode == "new" ) {
	if ( ! isset ($_POST['submittestsuite']) ) {
		...
		if ( ! isset ($_GET['srcts_id']) ) {
			...
			$sql = "SELECT id, auth_id, name, slogan, exemode, timeout FROM testsuite WHERE id=" . $_GET['srcts_id'];
			$result = &$db->doQuery($sql);
			if( mysql_num_rows($result) == 0)
			...
			#$ts_id=$row->id;
			$name=$row->name . " **Copy** ";
			$slogan= "**Copy** " . $row->slogan;
			$auth_id = $row->auth_id;
			$exemode = $row->exemode;
			$timeout = $row->timeout;
	
			# get TS paths
			$tspath = getTSDirPath( &$db, $_GET['srcts_id'] );
			$specfilepath = getTSSpecPath( $tspath, $_GET['srcts_id'] );
			$tssfilepath = getTSScriptPath( $tspath, $_GET['srcts_id'] );
			$selected_tcs = getSelectedTestcases(&$db, $_GET['srcts_id']);
			$available_tcs = getAvailableTestcases( &$db, getSelectedTestcases( &$db, $_GET['srcts_id'] ) );
		}
	}
...
function moveUp(theSelList)
{
  ...
  for(i=selLength-1; i>=0; i--)
  {
    if(theSelList.options[i].selected)
    {
      // don't move if already at top	
      if ( i == 0 ) {
        continue;
      }
      tmptext=theSelList.options[i-1].text;
      theSelList.options[i-1].text=theSelList.options[i].text
      theSelList.options[i].text=tmptext;
      tmpval=theSelList.options[i-1].value;
      theSelList.options[i-1].value=theSelList.options[i].value
      theSelList.options[i].value=tmpval;
      theSelList.options[i-1].selected=true;
      theSelList.options[i].selected=false;
...
function moveDown(theSelList)
{
  ...
  for(i=selLength-1; i>=0; i--)
  {
    if(theSelList.options[i].selected)
    {
      // don't move if already at bottom
      if ( i == selLength-1 ) {
        continue;
      }
      tmptext=theSelList.options[i+1].text;
...
function OnGenTestSpecButton(theSelList) {
  var argList='';
  for (var i=0; i<theSelList.options.length; i++) 
  {
	argList += theSelList.options[i].value + ",";
	
  } 
  if ( argList.length > 1 ) {
	argList = argList.substring(0, argList.length - 1);
  }
  window.location.href="./generate_testspec.php?genspec=1&tc_ids=" + argList;
