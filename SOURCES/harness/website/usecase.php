...
if ( $mode == "new" ) {
	...
	if ( ! isset ($_GET['srcuc_id']) ) {
		...
		$sql = "SELECT funcarea.name AS fa_name, funcarea.id as fa_id, usecase.name AS uc_name, usecase.id AS uc_id, slogan, steps, author.id as auth_id, author.signum as auth_signum FROM funcarea, usecase, author WHERE funcarea.id=usecase.fa_id AND author.id=usecase.auth_id AND usecase.id = " . $_GET['srcuc_id'];
		$result = &$db->doQuery($sql);
		if( mysql_num_rows($result) == 0)
...
if ( isset ($_POST['submitusecase']) &&  $_POST['submitusecase'] == "$submitbtntxt" ) {
	...
	} else if ( strpos($uc_name, $fa_name != 0 )   ) {
		$formerrors++;
