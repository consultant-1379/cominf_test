...
function OnViewButton()
{
	if ( document.testcaseform.tc_id.value == 0 ) {
		alert("Please select a testcase to view");
		return false;
	}
...
{
	if ( document.testcaseform.tc_id.value == 0 ) {
		alert("Please select a testcase to copy");
		return false;
	}
	document.testcaseform.action = "testcase.php?mode=new&srctc_id=" +  document.testcaseform.tc_id.value;
	document.testcaseform.submit();             // Submit the page
	return true;
}
...
{
	if ( document.testcaseform.tc_id.value == 0 ) {
		alert("Please select a testcase to edit");
		return false;
	}
...
{
	if ( document.testcaseform.tc_id.value == 0 ) {
		alert("Please select a testcase to delete");
		return false;
	}
...
function OnGenTestSpecButton(fa_id, uc_id)
{
        document.testcaseform.action = "generate_testspec.php?genspec=1&fa_id=" + fa_id + "&uc_ids=" + uc_id;
        document.testcaseform.submit();             // Submit the page
        return true;
}
...
$form->AddButton($fld);
$fld = new Button('copy', 'Copy','submit', '', 'return OnCopyButton();');
$form->AddButton($fld);
$fld = new Button('delete', 'Delete','submit', '', 'return OnDeleteButton();');
$form->AddButton($fld);
$fld = new Button('genspec', 'Generate Spec','submit', '', "return OnGenTestSpecButton($fa_id, $uc_id);");
$form->AddButton($fld);
